...
# Changelog
#
# 2010-04-04: v2.2a rajout de la flèche à la fin de la progression. (by Autiwa)
...
class Bar(ProgressBarWidgetHFill):
    ...
    def __init__(self, marker='=', left='[', right=']', progress_marker=''):
        self.marker = marker
        self.left = left
        self.right = right
    ...
    def update(self, pbar, width):
        ...
        cwidth = width - len(self.left) - len(self.right)-len(self.progress_marker)
        marked_width = int(percent * cwidth / 100)
        m = self._format_marker(pbar)
        bar = (self.left + (m*marked_width+self.progress_marker).ljust(cwidth) + self.right)
        return bar
